...
# ===============================
RESNET_PATH = "best_ResNet50.keras"
RESNET_URL = "https://github.com/Samah-Mahdi/CorroVision-AI/releases/download/v1.0.0/best_ResNet50.keras"

CUSTOM_CNN_PATH = "best_CustomCNN.keras" 

@st.cache_resource
def load_ai_models():
    # تحميل ResNet-50
    if not os.path.exists(RESNET_PATH):
        with st.spinner("جاري تنزيل نموذج ResNet-50 لأول مرة..."):
            urllib.request.urlretrieve(RESNET_URL, RESNET_PATH)
    resnet_model = load_model(RESNET_PATH)
    
    # تحميل Custom CNN (إن وجد)
...
# ===============================
# 3. دوال معالجة الصورة والتنبؤ
# ===============================
def predict_resnet(image, model_obj):
    img = image.resize((224, 224)).convert("RGB")
    img_array = np.array(img, dtype=np.float32)
    
    # معالجة خاصة بنموذج ResNet
    img_array = preprocess_input(img_array)
    img_array = np.expand_dims(img_array, axis=0)
    ...
    prob = float(pred_arr[0][0]) if (pred_arr.ndim == 2 and pred_arr.shape[1] == 1) else float(pred_arr.squeeze())
    
    has_corrosion = prob >= 0.35  
...
def predict_custom_cnn(image, model_obj):
    ...
    # التطبيع القياسي لنماذج CNN المخصصة (بدون preprocess_input)
    ...
    has_corrosion = prob >= 0.50 # الحد الفاصل الافتراضي للـ Custom CNN 
    confidence = round(prob * 100, 2) if has_corrosion else round((1 - prob) * 100, 2)
    return has_corrosion, confidence
...
if button_container.button(" بدء التحليل الذكي والمقارنة 🚀", use_container_width=True):
    if image_to_process is None:
        st.error("الرجاء رفع صورة الأنبوب أولاً لإتمام العملية.")
    elif resnet_model is None:
        st.error(f"لم يتم العثور على نموذج ResNet في المسار: {RESNET_PATH}")
    else:
        # إخفاء الزر فوراً
        button_container.empty()
        
        with st.spinner('جاري تحليل الصورة بواسطة النموذجين ومقاطعة النتائج مع بيانات الضغط...'):
            st.markdown("---")
            image = Image.open(image_to_process)
            
            # استخراج نتائج النموذجين
            has_corrosion_res, conf_res = predict_resnet(image, resnet_model)
            
            has_corrosion_cnn, conf_cnn = False, 0.0
            if custom_cnn_model is not None:
                has_corrosion_cnn, conf_cnn = predict_custom_cnn(image, custom_cnn_model)
            
            st.markdown("### 📊 تقرير الفحص والمقارنة")
            ...
            with res_col2:
                # عرض نتائج المقارنة في عمودين داخليين
                metrics_col1, metrics_col2 = st.columns(2)
                
                with metrics_col1:
                    st.info("🤖 **ResNet-50 (الأساسي)**")
                    status_res = "تآكل ⚠️" if has_corrosion_res else "سليم ✅"
                    st.metric(label=f"التصنيف: {status_res}", value=f"{conf_res}%")
                    
                with metrics_col2:
                    st.success("⚙️ **Custom CNN (مخصص)**")
                    if custom_cnn_model:
                        status_cnn = "تآكل ⚠️" if has_corrosion_cnn else "سليم ✅"
                        st.metric(label=f"التصنيف: {status_cnn}", value=f"{conf_cnn}%")
                    else:
                        st.warning("النموذج غير متوفر")
                
                st.markdown("---")
                
                # تطبيق المنطق المركب بناءً على نموذج ResNet الأساسي
